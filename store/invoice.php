...
<main class="px-4 pb-8 pt-4">
    <h1 class="text-2xl font-bold text-slate-900">Invoice pembayaran</h1>
    ...
    <section class="mt-5 rounded-[28px] border border-stone-200 bg-white p-5 shadow-soft">
        <div class="flex items-center justify-between gap-3">
            <div>
                ...
                <div class="text-lg font-bold text-slate-900"><?= e($orderCode) ?></div>
            </div>
        ...
        <div class="mt-5 grid gap-3 rounded-3xl bg-stone-100 p-4 text-sm">
        ...
        <?php if ($order): ?>
            <div class="mt-4 rounded-3xl border border-stone-200 p-4 text-sm text-slate-600">
            ...
            <?php if (!empty($order['tripay_qr_url'])): ?>
                <div class="mt-4 rounded-3xl border border-stone-200 p-4 text-center">
                    <div class="mb-3 text-sm font-semibold text-slate-700">Scan QR untuk bayar</div>
                    <img src="<?= e($order['tripay_qr_url']) ?>" alt="QRIS" class="mx-auto w-full max-w-[260px] rounded-2xl border border-stone-200">
                </div>
            <?php endif; ?>

            <?php if (!empty($order['tripay_pay_code'])): ?>
                <div class="mt-4 rounded-3xl border border-stone-200 p-4 text-sm text-slate-700">
            ...
            <?php if (!empty($order['tripay_checkout_url'])): ?>
                <a href="<?= e($order['tripay_checkout_url']) ?>" target="_blank" class="mt-4 inline-flex w-full justify-center rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white">Buka checkout Tripay</a>
            <?php endif; ?>

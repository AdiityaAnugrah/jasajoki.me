...
<main class="min-h-screen bg-slate-100">
    <div class="mx-auto grid min-h-screen max-w-[1600px] xl:grid-cols-[280px_1fr]">
        <aside class="hidden border-r border-slate-200 bg-slate-950 text-white xl:block">
            <div class="sticky top-0 p-6">
                <div class="rounded-3xl bg-white/5 p-5">
                    <p class="text-xs font-semibold uppercase tracking-[0.25em] text-blue-200">JasaJoki Admin</p>
                ...
                <nav class="mt-6 space-y-2">
                    <?php foreach ($adminNav as $item): ?>
                        <a href="<?= e($item['href']) ?>" class="flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-semibold <?= $activeNav === $item['key'] ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' ?>">
        ...
        <section class="p-4 md:p-6 xl:p-8">
            <header class="mb-6 flex flex-col gap-4 rounded-3xl bg-white p-5 shadow-soft md:flex-row md:items-center md:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.22em] text-blue-600">Admin workspace</p>

...
<section class="grid gap-6 2xl:grid-cols-[420px_1fr]">
    <form method="post" enctype="multipart/form-data" class="admin-panel space-y-5 p-6">
        ...
        <div>
            <p class="admin-panel-kicker">Produk digital</p>
            <h3 class="mt-2 text-2xl font-black text-accent-900"><?= $editing ? 'Edit produk' : 'Tambah produk' ?></h3>
            <p class="admin-panel-subtitle mt-2">Bikin produk baru atau rapikan produk lama dengan form yang lebih nyaman dipakai.</p>
        </div>
        <?php if ($success): ?>
        ...
        <div class="admin-field-group">
            <label class="text-sm font-semibold">URL gambar produk</label>
            <input name="image_url" value="<?= e($editing['image_url'] ?? '') ?>" class="admin-input" placeholder="https://foto.jasajoki.me/products/...">
            <p class="admin-field-help">Bisa isi manual atau upload file langsung ke R2 di bawah.</p>
        </div>
        <div class="admin-field-group">
            <label class="text-sm font-semibold">Upload gambar ke R2</label>
            <input type="file" name="image_file" accept=".jpg,.jpeg,.png,.webp" class="admin-input">
            <p class="admin-field-help">Format: jpg, jpeg, png, webp.</p>
        </div>
    ...
    <div class="admin-panel p-6">
        <div class="mb-5 flex items-center justify-between gap-4">
            <div>
                <p class="admin-panel-kicker">Inventory katalog</p>
                <h3 class="mt-2 text-xl font-black text-accent-900">Daftar produk</h3>
                <p class="text-sm text-slate-500">Produk aktif dan nonaktif sekarang lebih mudah discan secara visual.</p>
            </div>
            <div class="admin-filter-chip"><?= count($products) ?> produk</div>
        </div>
                    ...
                    <tr>
                        <td>
                            <div class="flex items-center gap-3">
                                ...
                                <?php else: ?>
                                    <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-50 text-sm font-black text-sky-700"><?= e(strtoupper(substr($product['name'], 0, 1))) ?></div>
                                <?php endif; ?>
                                <div>
                        ...
                        <td>
                            <span class="admin-status-chip <?= !empty($product['is_active']) ? 'bg-emerald-50 text-emerald-700' : 'bg-stone-100 text-slate-700' ?>">
